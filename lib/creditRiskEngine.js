...
function calculateCreditScoreProxy(data) {
  const {
    ...
    creditMix = 1,            // Number of different credit types
  ...
  if (paymentHistory.length > 0) {
    ...
    score += Math.round(onTimeRate * 210);
  } else {
  ...
  if (creditUtilization <= 10) utilizationScore = 180;
  else if (creditUtilization <= 30) utilizationScore = 150;
  else if (creditUtilization <= 50) utilizationScore = 100;
  else if (creditUtilization <= 70) utilizationScore = 50;
  ...
  const mixScore = Math.min(60, creditMix * 15);
  score += mixScore;

  // 5. Recent Inquiries (10% weight) - up to 60 points
  let inquiryScore = 60;
  if (recentInquiries >= 5) inquiryScore = 10;
  else if (recentInquiries >= 3) inquiryScore = 30;
  else if (recentInquiries >= 1) inquiryScore = 45;
  score += inquiryScore;
