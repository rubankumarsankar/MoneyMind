...
function simulateCashFlow(data, months = 6) {
  ...
  for (let m = 1; m <= months; m++) {
    ...
    if (data.overrides && data.overrides[m-1]) {
        const override = data.overrides[m-1];
        
        // Granular Overrides: Update specific values if provided, otherwise keep calculated
        if (override.income !== undefined && override.income !== null && override.income !== '') {
            currentIncome = Number(override.income);
        }
        
        // Expense Override: Replaces standard calculation logic if provided
        // We assume providing an expense override means "I know exactly what I'll spend this month"
        if (override.expenses !== undefined && override.expenses !== null && override.expenses !== '') {
             const overrideExpense = Number(override.expenses);
             const totalExpense = overrideExpense; // Overrides fixed + variable + etc.
